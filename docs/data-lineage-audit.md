...
| 35 | Goods Receipt storage locations / batches pick lists | GoodsReceiptAdapter.js:195-204,285-295; goodsReceipt.handler.js:26-58 | a - live | `getMaterialStorageLocations` and `getMaterialBatches` rethrow on S/4HANA outages and backend errors instead of swallowing and returning `[]`. CAP handlers in `goodsReceipt.handler.js` reject with 502, surfacing SAP failures to callers and UI. | High | Surface the error; drop the dead source. | RESOLVED |
...
| 35 GR pick list errors surfaced | Fixed | `getMaterialStorageLocations` and `getMaterialBatches` rethrow on S/4HANA outages and backend errors instead of swallowing and returning `[]`. CAP handlers reject with 502. |
