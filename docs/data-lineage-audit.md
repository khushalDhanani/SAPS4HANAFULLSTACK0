...
| 31 | Approval status shown on due orders / sales orders | OutboundDeliveryAdapter.js:192-225,280-340; service.cds:20; OrdersDueForDelivery/SalesOrders views & controllers | a/b - live / cached 60 s | `SD_F1873_SO_WL_SRV` lookup. On lookup failure, cache is invalidated and status returned as `null` -> mapped as `'unknown'`. Create Delivery button disabled in UI (views & controllers block with warning); backend `createDeliveryFromOrder` blocks with HTTP 502 (if lookup failed) or HTTP 400 (if unapproved). | High | Mark status 'unknown' and block delivery creation across UI and backend. | RESOLVED |
