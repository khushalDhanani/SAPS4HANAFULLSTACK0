...
| 4 | PO status text (Approved / Draft / In Approval / Rejected) | webapp/model/formatter.js:6-45 | a - live | Verified 33 status codes against SAP S/4HANA Gateway (`C_PURCHASEORDER_FS_SRV/I_PurchasingDocumentStatusText`). Shows authentic SAP status name (`sStatusName`); deletion flag 'L' maps to 'Deleted'; status codes mapped to verified SAP names or raw codes. All synthetic boolean fallbacks (`completeness = true -> Approved`, `completeness = false -> Draft`, `releaseNotCompleted -> In Approval`) eliminated. | High | Show SAP's status name only; verify codes against SAP status value help. | RESOLVED |
...
| 19 | Sales Order worklist rows | SalesInquiryAdapter.js:825-965 | a - live | CQN/OData translation implemented. HTTP fallback preserves pagination ($top, $skip), ordering ($orderby), inlinecount, and translates filters (SoldToParty, status, contains/substringof, operators, parentheses) to OData V2 $filter. If filter translation fails, throws HTTP 500 refusing to return unfiltered data. | High | Apply the same filter in the fallback or fail. | RESOLVED |
...
| 21 | Sales Order / Inquiry KPI 'Open ...' and 'Active Customers' | SalesOrders.controller.js:100-118; SalesInquiries.controller.js:72-89 | a - live | Page-only scraping eliminated. Unifies Open definition to S/4HANA OverallSDProcessStatus ne 'C' via getSalesMetrics ($inlinecount on C_InquiryWL_F2370 with open-status filter). openOrdersCount fallback eliminated in SalesInquiries.controller.js:81-85; Open Inquiries tile strictly binds openInquiriesCount and never shows sales order counts. Fetches authentic server customer count via getDashboardMetrics() (cached master data). | High | Use the server counts (`getSalesMetrics`) and one definition. | RESOLVED |
...
| 24 | Sales order / inquiry header defaults (type, sales org, channel, division, plant, currency, dates) | SalesInquiryAdapter.js:849-870,1032-1050; SalesOrderModel.js:54-127,475-524; SalesOrderService.js:300-365; CreateSalesOrder.controller.js:46-88,285 | d - config + hardcoded | Hardcoded literal `ZDOM`,`1000`,`INR`,`KG`,`1120`, plant `"1000"`, `quantity \|\| 1`, `delivery date today + 7` eliminated across `SalesOrderModel.js` (`createInitialModel`, `createEmptyItem`, `applyMaterialDefaults`, `buildPayload`), `SalesOrderService.js` (`getSalesOrderDefaults`, `getCustomerDefaults`), and `CreateSalesOrder.controller.js` (`_loadConfigurationAndDefaults`, `onAddItem`). Defaults sourced exclusively from server `getSalesOrderDefaults()` via `applyServerDefaults()`. Empty on failure. | High | Already listed in docs/no-assumed-data-changes.md sections 3.2-3.8. | RESOLVED |
...
| 30 | Orders Due KPI 'Due Orders' | OrdersDueForDelivery.controller.js; OrdersDueForDelivery.view.xml; service.cds | a - live | Counts rows of schedule lines entity via `getOrdersDueMetrics()`. Segmented into 'Ready to Deliver' (good state) and 'In Approval' (critical state) alongside total schedule lines. Display count dynamically tracks active tab filter. Table defaults to deliverable orders (`ready`). | High | Relabeled, added segmented KPIs and default deliverable filtering. | RESOLVED |
| 31 | Approval status shown on due orders / sales orders | OutboundDeliveryAdapter.js:192-235,280-355; service.cds:20; OrdersDueForDelivery/SalesOrders views & controllers | a/b - live / cached 60 s | `SD_F1873_SO_WL_SRV` lookup. Fixed filter `SalesDocApprovalStatus ne ''` so status 'B' (Released) is included in map. Added `IsDeliverable` boolean. Worklist segmented into 'Ready to Deliver' (default), 'In Approval', and 'All'. On lookup failure, cache is invalidated and status returned as `null` -> mapped as `'unknown'`. Create Delivery button disabled in UI (views & controllers block with warning); backend `createDeliveryFromOrder` blocks with HTTP 502 (if lookup failed) or HTTP 400 (if unapproved). | High | Fixed status 'B' query, added IsDeliverable, segmented UI worklist. | RESOLVED |
...
| 33 | Goods Receipt scan resolution (delivery, PO, items, material, supplier, plant) | GoodsReceiptAdapter.js:450-715; goodsReceipt.handler.js:71-77; GoodsReceipt.controller.js:296-318 | a - live | Tiered reads of `HMmimGr4inbdelSet`, `PoHelpSet`, `I_Batch`. S/4HANA backend outage detection (`_isOutage`) implemented across all lookup tiers (Tiers 1-6). Outages rethrow immediately with 502/503/504; handler rejects with 502/503/504 and UI surfaces backend outage dialog instead of masking as 404 'barcode does not exist'. | Med | Distinguish outage from not-found (as GoodsIssueBatchesClient._isOutage does). | RESOLVED |
| 34 | Goods Receipt proposed quantity / open / ordered / unit | GoodsReceiptAdapter.js:300-425,730-775; GoodsReceipt.controller.js:44,275,490; GoodsReceipt.view.xml:209 | a - live | `GR4PO_DL_Items.OpenQuantity` etc. Authentic `null` values returned on missing/failed item reads instead of coercing to `0`. `Quantity`, `OpenQuantity`, `OrderedQuantity`, and `QuantityInEntryUnit` set to `null` on read failure. Controller preserves `""` and `null`; view expression binds `!== null && !== ''`. | Med | Return `null` on failed read. | RESOLVED |
| 35 | Goods Receipt storage locations / batches pick lists | GoodsReceiptAdapter.js:175-205,820-840; goodsReceipt.handler.js:26-58 | a - live | Dead source `MMIM_MATERIAL_DATA_SRV/MaterialStorLocHelps` (0 rows) dropped. Replaced with live SAP Storage Location Value Help `MM_PUR_PO_MAINT_V2_SRV/C_MM_StorLocValueHelp` (696 rows in SAP) filtered by Plant. Authentic document `StorageLocation` and name from `GR4PO_DL_Items` pre-selected and guaranteed in `AvailableStorageLocations`, eliminating the empty picker bug. Rethrows on S/4HANA outages and backend errors. | High | Surface the error; drop the dead source. | RESOLVED |
...
| 37 | Goods Issue reservation list | GoodsIssueReservationsClient.js:45-145; goodsIssue.handler.js:23-38; GoodsIssue.view.xml; GoodsIssue.controller.js | a - live | Server-side filtering by Reservation and OrderID implemented in SAP OData filter before paging; truncation explicitly detected with LOG.warn, IsTruncated flag, ItemCountPartial indicator for cutoff reservation, TruncationNote, and MessageStrip / toast in UI informing user when first 2,000 items are scanned. | High | Filter server-side by plant/order before paging, or show 'first 2,000'. | RESOLVED |
...
|---|---|---|---|
| Substitute a value that looks like data | 8 | salesOrder.handler.js:92 (0/0); GoodsIssueBatchesClient.js:39,75,96,143 and GoodsIssueStockUnitClient.js:627,642 (stock becomes 0, units become synthetic); GoodsIssueReservationsClient.js:188 ('NO SLED') | rows 22, 40, 41, 42 |
| Return an empty list | 1 | SalesInquiryAdapter.js:1440 | row 35 [RESOLVED - GoodsReceiptAdapter throws on outage/error and handler rejects with 502] |
| Fall through to the next lookup tier | 0 | GoodsReceiptAdapter.js [RESOLVED] | row 33 [RESOLVED - _isOutage rethrows immediately across Tiers 1-6 with 502/503/504] |
| Switch to another read path | 3 | SalesInquiryAdapter.js (getSalesOrders fallback) | row 19 [RESOLVED]; fallback preserves user's filter/pagination/ordering and throws 500 if filter cannot be translated |
...
| 19 Sales Order HTTP fallback filter | Fixed | CQN where/orderby/limit AST translated to OData v2 ($filter, $orderby, $top, $skip, $inlinecount). Filter strictly preserved in fallback; throws error refusing to return unfiltered results if filter cannot be translated. |
...
| 24 Sales header defaults in the UI | Fixed | Hardcoded org/currency/plant/unit/qty defaults eliminated from `createInitialModel`, `createEmptyItem`, `buildPayload`, and `getSalesOrderDefaults`. Defaults sourced exclusively from server `getSalesOrderDefaults()` via `applyServerDefaults`. Empty on failure. |
...
| 31 Approval status on due orders | Fixed | Lookup failure marks status as 'unknown', creates warning message strip, and blocks delivery creation across UI and backend. |
...
| 37 GI reservation list 2,000 cap | Fixed | Pushes Reservation/Order server-side into SAP OData $filter before paging; non-silent truncation with LOG.warn, IsTruncated flag, ItemCountPartial indicator, and UI MessageStrip/toast. |
