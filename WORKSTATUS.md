...
- **Change**: Removed every dashboard figure that was not read from SAP S/4HANA, the invented sales order and inquiry-type fallbacks, and the placeholder tabs and simulators (`srv/integration/s4hana/mm/purchase-order/PurchaseOrderAdapter.js`, `srv/mm/purchase-order/handlers/purchaseOrder.handler.js`, `srv/integration/s4hana/sd/sales-inquiry/SalesInquiryAdapter.js`, `srv/sd/sales-inquiry/handlers/salesInquiry.handler.js`, `srv/sd/sales-inquiry/service.cds`, `app/fiori-app/webapp/view/Dashboard.view.xml`, `app/fiori-app/webapp/controller/Dashboard.controller.js`, `app/fiori-app/webapp/service/ValueHelpService.js`, `app/fiori-app/webapp/i18n/i18n.properties`, `app/fiori-app/webapp/i18n/i18n_en.properties`, `test/unit/dashboard/dashboardMetrics.test.js`, `test/unit/sales-inquiry/salesInquiryAdapter.test.js`, `WORKSTATUS.md`).
  - **Root cause**: the review named two adapter fallbacks and the placeholder tabs. Tracing the dashboard end to end showed the problem was wider:
    1. `SalesInquiryAdapter.getSalesMetrics` returned fixed counts 498 open / 880 total when the destination could not be resolved, and 0 / 0 when the SAP call failed.
    2. `SalesInquiryAdapter.getInquiryTypes` returned a built-in list of ten inquiry types when SAP could not be read, and for real SAP rows replaced SAP's own texts with invented descriptions, "classifications" and "business purpose" paragraphs; missing procedures and screen sequence groups were defaulted (`01`, `TA`, `AG`). The value help dialog displayed and filtered on those invented fields.
    3. `PurchaseOrderAdapter.getDashboardMetrics` turned every failed count into 0, counted rows when SAP returned no count, extrapolated "procurement volume" from an arbitrary 100 purchase orders across mixed currencies, and computed a "completeness rate" from the same sample (100 % when nothing came back).
    4. Of the 60 model properties bound in `Dashboard.view.xml`, 34 were never loaded by anything (HCM headcount, TM routes, PP capacity, QM lots, EAM orders, PS WIP, car loans, analytics cash flow and DSO, admin jobs, MDG change requests, work centers, bank accounts, cost settlement, EWM task counts, …), so those tiles permanently showed 0 as if it were live. `systemHealth` was hard-coded to 100 % and the header always showed "Client 220 • S/4HANA Active".
    5. The car loan "simulator" showed a hard-coded EMI calculation and "New Loan Application" announced application `LA-2026-089` and "workflow routing initiated" without any backend. The CO, PP, QM, EAM, PS, TM, Service, HCM, Analytics and Admin tabs had no working app behind any tile, and the 20 master data tiles opened a dialog claiming "Official SAP Catalog Entry … Verified in S/4HANA Catalog (DS4 / Client 220)".
    6. Several remaining tiles were mislabelled (customer value-help count shown as "Customer Balances / Active Debtors" and "Customer 360"; cost center count shown as "Actual Cost Rate"; warehouse count shown as "Outbound Deliveries"). When the unified call failed, the controller fired 22 separate requests whose failures were silently swallowed.
  - **Resolution**:
    1. `getDashboardMetrics`: 26 counts, each read with `=allpages&=1` (or `/`). A count is a number only when SAP returned one; otherwise it is `null` and listed in `unavailable`. Volume estimate and sampled completeness rate removed. The handler returns 503 with the SAP message when the destination cannot be resolved. `getBusinessPartnerCount` returns `null` instead of 0.
    2. `getSalesMetrics`: throws 503 without a destination, 502 when SAP fails or returns no count; `getSalesOrderMetrics` passes the error through.
    3. `getInquiryTypes`: no built-in list and no invented texts. SAP descriptions, number ranges, procedures and screen sequence groups pass through unchanged (`null` when absent); only the active status (from `IsLocked`) and the category name of category A are derived. The WL value help is read only when the factsheet service returns nothing; if both fail the call throws 502, and 503 when neither service is connected. `Classification` and `Purpose` removed from `SalesInquiryTypeVH`; the value help dialog lost those two columns, the category filter and the matching search fields.
    4. Dashboard view: 6 tabs (Overview, Master Data, FI, MM, SD, Warehouse) instead of 16. Every tile either opens a working app or shows one of the 26 live counts, and every metric tile shows `Loading` until its count arrives and `Failed` when SAP did not return it. CO, PP, QM, EAM, PS, TM, Service, HCM, Analytics and Admin tabs, the car loan simulator and application button, the master data info dialog, the hard-coded health tile and client badge were removed. The header status now reports "S/4HANA connected", "partially available (n of 26 figures)" or "not reachable" from the actual result, and a message strip shows the SAP error when the call fails. Mislabelled tiles were relabelled; WBS elements moved into Financial master data; "All Sales Orders" added from the already-loaded total count.
    5. Controller: one metrics call, no client-side fallback; metrics are `undefined` while loading, `null` when unavailable; car loan, info dialog and placeholder tab handlers removed.
    6. i18n (both files): 61 keys orphaned by the removed tabs, simulator and KPIs deleted (only keys with no remaining reference); 15 texts that described features the dashboard never had corrected (e.g. FI "customer receivables, bank accounts", SD "quotations, 360 customer intelligence", EWM "picking carts", master data "Governance / MDG"); 7 keys added for connection status and error messages.
    7. Tests: `dashboardMetrics.test.js` rewritten (26 tests: sales metrics fail instead of inventing counts; controller starts with no figures, populates SAP counts, keeps unavailable counts `null`, rejects non-count values, reports connected / partial / unreachable, makes no fallback calls, clears figures on refresh; formatters; no simulator or placeholder handlers; the view binds only live metrics or status fields, every metric tile uses the Loading/Failed state, only the six tabs exist, every press handler exists; adapter reads 26 live counts, drops spend/rate, returns `null` per failed or count-less call, keeps a genuine 0, propagates a missing destination). The two inquiry-type adapter tests that asserted invented classifications and the baseline list were replaced by four tests (SAP texts pass through unchanged, WL used only when FS is empty, 502 when both fail, empty list on an empty SAP answer).
  - **Validation**:
    - `npm test` (unit + integration + e2e): **61 suites, 775 tests, all passed**.
    - `npx ui5lint`: no findings. `npm run build` (`app/fiori-app`): succeeded. `xmllint --noout` on the dashboard view: well-formed (only the usual UI5 `sap.m` namespace warning, also present before).
    - `npx cds compile srv --to json`: 0 errors. `git diff --check`: clean.
    - Live check against DS4 client 220 through the developer's running `cds watch` on port 4004, as the mocked dev user (read-only GETs): `getDashboardMetrics()` HTTP 200 with 26 of 26 figures and `unavailable: []`, no `totalSpend` / `completeRate`; `getSalesOrderMetrics()` HTTP 200 with open 498 / total 880 (the removed hard-coded fallback matched a real snapshot); `SalesInquiryTypeVH` HTTP 200 with 10 types and SAP's own texts (e.g. ZIN "Inquiry", VLAF blank), no `Classification` / `Purpose`.
  - **Not validated**: the rebuilt dashboard was not viewed in a browser (login would require entering a password), so the rendering of `Loading` / `Failed` tiles and the status badge was checked only through lint, build and unit tests. The failure paths were exercised with mocks, not by breaking the live destination.
  - **Behaviour change for users**: the dashboard is smaller (6 tabs); inquiry types show SAP's own descriptions, which for some types are generic ("Inquiry") or empty; when S/4HANA is unreachable users now see failed tiles and an error instead of zeros.
  - **Findings recorded for follow-up** (not changed here): `SalesInquiryAdapter.getCustomerDefaults` pre-sets `sCurrency = 'INR'`, so the currency read from the customer's inquiries is never used, and defaults the country to `IN`; `getSalesInquiryDefaults` returns hard-coded sales area and currency (`ZIN`, `1000`, `10`, `52`, `INR`) flagged `derived: true`; `BaseController.calculateKpiMetrics` computes a completeness rate from the rows currently loaded and reports the order total as the supplier count when no supplier is found.
  - **Next recommended action**: review item 8 (two overlapping authentication systems: custom S/4 password login next to XSUAA, unused `/auth/` approuter route, dev token issuer with a hard-coded fallback secret).

## 2026-09-15 09:30 IST
- **Agent**: Claude Code (Opus 5)
- **Change**: Removed every local substitute for SAP EWM transactions from the warehouse management module; SAP rejections are now returned to the user as errors (`srv/ewm/warehouse-management/handlers/warehouseManagement.handler.js`, `srv/integration/s4hana/ewm/EwmAdapter.js`, `srv/ewm/warehouse-management/service.cds`, `app/fiori-app/webapp/modules/ewm/warehouse-cockpit/controller/CreateWarehouseTask.controller.js`, `app/fiori-app/webapp/modules/ewm/warehouse-cockpit/view/WarehouseCockpit.view.xml`, `test/unit/ewm/warehouseManagementLocalStaging.test.js` removed, `test/unit/ewm/warehouseManagementSapOnly.test.js` new, `test/unit/ewm/ewmAdapter.test.js`, `test/unit/ewm/createWarehouseTask.test.js`, `test/unit/ewm/rfTerminal.test.js`, `WORKSTATUS.md`).
  - **Root cause**: when SAP rejected warehouse task creation, `createWarehouseTask` stored an invented task (`WT-1000n`, order `WO-<warehouse>`, `_isLocalStaging: true`) in a process-local Map and returned it as a created task. The review flagged that one path; tracing it end to end found the same pattern in eight more places, all contradicting the AGENTS.md rule against local substitutes for SAP transactions and ADR-0001:
    1. `READ WarehouseTasks` and `READ WarehouseKPIs` merged the invented tasks into SAP data and counted them as open tasks.
    2. `confirmWarehouseTask`, `cancelWarehouseTask` and `confirmRfPick` changed invented tasks in memory and returned `true` without calling SAP.
    3. `postGoodsReceipt` and `postGoodsIssue` returned `true` when SAP rejected the posting, and the delivery reads then showed those deliveries as completed (`C`) although SAP never posted them. Both also defaulted a missing warehouse to `W22`.
    4. `READ WarehouseProcessTypes` returned three invented process types (`1010/2010/3010 … (Local Staging)`) when SAP had none.
    5. `EwmAdapter.createWarehouseTask` "strategy 3" posted a real goods receipt on an arbitrary open inbound delivery as a side effect of creating a task, then returned an invented `GR-<delivery>` task if no task appeared.
    6. `EwmAdapter.logonResource` swallowed a rejected SAP logon and still returned `LogonStatus: 'ACTIVE'`.
    7. The create-task screen invented process types, storage types and bins for warehouses with no EWM configuration, defaulted the process type to `1010` in the loader, the validator and the payload, and told the user "Local Staging persistence will be automatically used".
  - **Resolution**:
    1. Handler: deleted the module-level Maps/Sets and the `resetLocalStaging` / `getLocal*` helpers. Reads return exactly what SAP returns. Create, confirm, cancel, RF confirm, goods receipt and goods issue call SAP and, on rejection, answer `req.error(err.status || 500, err.message)`. `createWarehouseTask` requires `WarehouseProcessType` (400) instead of defaulting `1010`. Goods receipt and goods issue require the warehouse and the document (400) instead of defaulting `W22`.
    2. Adapter: removed strategy 3 and its helper; creation tries only the two real create services and otherwise throws 422 "all SAP strategies exhausted; no task was created". `logonResource` propagates the SAP rejection.
    3. `service.cds`: removed `_isLocalStaging` from `WarehouseTasks`.
    4. UI: the cockpit no longer shows a "Local Staging" label; the create-task success message always refers to SAP; the create screen shows only SAP master data (possibly empty), marks Warehouse Process Type as required, and the warning now says SAP will reject task creation until process types are configured in /SCWM/T333.
    5. Tests: the 12-test local staging suite was replaced by `warehouseManagementSapOnly.test.js` (20 tests: no staging helpers exist; create returns SAP's task or SAP's rejection with its status; process type required; reads, KPIs and delivery statuses reflect SAP only after rejected writes; confirm/cancel/RF confirm always call SAP and propagate rejections; goods receipt and goods issue return true only on SAP success and require warehouse and document). Adapter tests: strategy 3 success tests replaced by a test proving no goods receipt, delivery read or task read happens during creation. RF logon tests now mock SAP and add a rejection case. Create-task UI tests assert no invented master data, a required process type that blocks submission, and SAP-only success wording.
  - **Validation**:
    - `npm test` (unit + integration + e2e): **61 suites, 755 tests, all passed**.
    - `npx ui5lint` (`app/fiori-app`): no findings.
    - `npx cds compile srv --to json`: 0 errors.
    - Repository sweep for `_isLocalStaging`, `Local Staging`, `localStagedTasks`, `localGoodsReceipts`, `localGoodsIssues`, `_goodsReceiptTriggered`, strategy 3 and the logon fallback across `srv`, `app/fiori-app/webapp` and `test`: none left outside assertions that check for their absence.
    - `git diff --check`: clean.
  - **Behaviour change for users**: on this S/4HANA instance `API_WAREHOUSE_ORDER_TASK` is deprecated and `PICKCART_SRV/WarehouseTaskSet` does not accept creates, so creating a warehouse task from the cockpit now fails with the SAP rejection instead of showing a task that never existed. The same applies to warehouse goods receipt / goods issue postings and RF logons that SAP rejects. Warehouse tasks must be produced by the SAP business process (goods receipt, goods issue, transfer posting) until a working create API is provisioned.
  - **Not validated**: no live SAP EWM call was made for these write paths; all rejections were exercised with controlled adapter mocks.
  - **Next recommended action**: review item 7 (dashboard figures invented when S/4 is unreachable, hard-coded inquiry-type catalogue, placeholder tiles such as the car loan simulator).

## 2026-09-14 16:22 IST
- **Agent**: Claude Code (Fable 5.1)
- **Change**: Provisioned SAP HANA Cloud (HDI container) as the production store for the Goods Issue dispatch queue and stopped generating database tables for the S/4-proxied service entities (`package.json`, `package-lock.json`, `mta.yaml`, `srv/ewm/warehouse-management/service.cds`, `srv/wm/goods-issue/service.cds`, `srv/wm/goods-receipt/service.cds`, `db/schema.cds`, `docs/decisions/ADR-0001-s4-centric-integration-facade.md`, `docs/architecture/ARCHITECTURE.md`, `README.md`, `WORKSTATUS.md`). Resolves the blocker recorded in the 16:16 IST entry (store decision: SAP HANA Cloud, chosen by the project owner).
  - **Resolution**:
    1. `npx cds add hana`: added `@cap-js/hana ^3` (installed 3.1.0; peer `@sap/cds ^10` matches the installed 10.0.6; `hdb` driver, no `@sap/hana-client`). No explicit `cds.requires.db` needed: the plugin defaults resolve `production -> hana`, while `development` and `test` keep `@cap-js/sqlite` in-memory, so local work and the test suites are unchanged.
    2. `mta.yaml`: new resource `saps4hana-db` (`com.sap.xs.hdi-container`, service `hana`, plan `hdi-shared`), new module `saps4hana-db-deployer` (`hdb`, path `gen/db`), and `saps4hana-srv` now requires `saps4hana-db`. The scaffold's `saps4hanafullstack-*` names were normalised to the project's `saps4hana-*` convention and a cosmetic reformat of `supported-platforms` was reverted.
    3. `@cds.persistence.skip` on every service-local entity that is read live from S/4HANA by a custom handler (12 EWM entities, `GIItems`/`MaterialBatches`/`OpenReservations` in Goods Issue, `OpenInboundDeliveries`/`MaterialStorageLocations`/`MaterialBatches` in Goods Receipt). Before this, `cds build --production` generated an `.hdbtable` for each of them, so the HDI container would have received about twenty empty tables that nothing writes to.
    4. Documentation: ADR-0001 gained an addendum (S/4 stays the system of record; the HDI container holds only the dispatch queue; behaviour without a bound database), `db/schema.cds` header corrected, `ARCHITECTURE.md` persistence row plus MTA module/resource lists updated, README prerequisites, managed services and build description updated.
  - **Validation**:
    - `npm ls`: `@cap-js/hana@3.1.0` with `@cap-js/db-service@3.1.0` and `hdb@2.29.6`; `npm audit --omit=dev`: 0 vulnerabilities.
    - `cds env requires.db`: production `{ kind: hana, impl: @cap-js/hana }`; development and test `{ kind: sqlite, url: ':memory:' }`.
    - `npx cds build --production`: `gen/db` now contains exactly `saps4hana.wm.GoodsIssueQueue.hdbtable`, `saps4hana.wm.GoodsIssueService.GoodsIssueQueue.hdbview`, CAP's standard `cds.outbox.Messages.hdbtable`, `.hdiconfig`, `.hdinamespace`; `gen/db/package.json` uses `@sap/hdi-deploy ^5` with `hdb`; `gen/srv/package.json` carries `@cap-js/hana`.
    - `mbt validate`: no errors. `npx cds compile srv --to json`: 0 errors; the only persisted application entity is `saps4hana.wm.GoodsIssueQueue`.
    - `npm test` (unit + integration + e2e): **61 suites, 751 tests, all passed** after the annotations.
    - Production-profile boot probe with an XSUAA binding but no HDI binding: `cds-serve` stops with `Database kind "hana" configured, but no HDI container or Service Manager instance bound to application`. Expected fail-closed behaviour; the MTA binds `saps4hana-db` to `saps4hana-srv`.
    - `git diff --check`: clean.
  - **Not validated**: no `cf deploy`; the HDI deployment and the queue running on a real HANA Cloud instance have not been observed. Deployment requires a HANA Cloud instance mapped to the target space; without one, `cf deploy` fails at creation of `saps4hana-db`.
  - **Next recommended action**: deploy to a space with HANA Cloud mapped, confirm `saps4hana-db-deployer` completes and `GET /odata/v4/goods-issue/getQueueSummary()` returns `StoreAvailable: true`; then continue with review item 6 (EWM handler fabricating warehouse tasks on SAP rejection).

## 2026-09-14 16:16 IST
- **Agent**: Claude Code (Fable 5.1)
- **Change**: Moved the Goods Issue dispatch queue from a JSON file under the user's home directory into the CAP database, and made queueing fail closed when no database is bound (`srv/wm/goods-issue/GoodsIssueQueueManager.js`, `srv/wm/goods-issue/handlers/goodsIssue.handler.js`, `srv/wm/goods-issue/service.cds`, `test/unit/wm/goodsIssueQueueManager.test.js`, `test/unit/wm/goodsIssueService.test.js`, `test/integration/wm/goodsIssueQueue.test.js` new, `data/goods-issue-queue.json` removed, `WORKSTATUS.md`). **Status: Resolved** at 16:22 IST (SAP HANA Cloud HDI container provisioned; see the following entry).
  - **Root cause**: `GoodsIssueQueueManager` persisted real stock movements (movement 261) to `~/.saps4hana/goods-issue-queue.json` (override `GI_QUEUE_STORAGE_FILE`). On Cloud Foundry the container disk is ephemeral and `mta/extensions/prod/prod.mtaext` runs two srv instances, so every instance would have kept its own queue and any restart or redeploy would have dropped it. At the same time `db/wm/goods-issue-queue.cds` already modelled `saps4hana.wm.GoodsIssueQueue` as a persisted entity while the production profile resolved no database (`cds env requires.db` undefined) and `mta.yaml` provisioned none. A tracked legacy store `data/goods-issue-queue.json` (25 QUEUED records) was still merged on startup.
  - **Resolution**:
    1. `GoodsIssueQueueManager` now reads and writes `saps4hana.wm.GoodsIssueQueue` through `cds.db` (`INSERT`/`SELECT`/`UPDATE`/`DELETE`, newest first by `QueuedAt`). All operations are async. `isAvailable()` is true only when CAP has bound a database; without one, reads return empty results and `getSummary()` reports `StoreAvailable: false`, while `enqueue`/`update`/`remove`/`clear` throw `QueueStoreUnavailableError` (status 503, code `GI_QUEUE_STORE_UNAVAILABLE`). File storage, `GI_QUEUE_STORAGE_FILE`, the home-directory store and the legacy migration are gone. Record shape and `GI-QUEUE-<reservation>-<item>-<nnnn>` references are unchanged; the hard-coded defaults plant `1120`, storage location `CS01`, unit `KG` and difference storage type `999` were removed from the record builder (values come from the request only).
    2. Handlers (`goodsIssue.handler.js`): `postGoodsIssue` awaits `enqueue` and, if the store is unavailable, returns the original SAP error with the suffix "The transaction could not be recorded in the dispatch queue either: … NOT recorded" instead of a fabricated success. The queued success message no longer says "safely recorded". `retryQueuedGoodsIssue` and `clearQueuedGoodsIssue` answer 503 when no store is bound. `READ GoodsIssueQueue` delegates to CAP's generic database handler (full OData `$filter`/`$count`/`$select` support) and returns `[]` without a store.
    3. `service.cds`: `GoodsIssueQueue` projection is now `@readonly` (all changes go through the three actions); `QueueSummary` gained `TotalCount` and `StoreAvailable`.
    4. Removed the tracked legacy file `data/goods-issue-queue.json` and the empty `data/` directory. The developer-local `~/.saps4hana/goods-issue-queue.json` is no longer read; its 25 dev records were never SAP-persisted and can be re-queued from the UI if still wanted.
    5. Tests: `goodsIssueQueueManager.test.js` rewritten against the in-memory SQLite database of the test profile (`cds.test`), covering availability, cross-instance visibility, update/remove/summary, and the handler paths (queue when available, fail closed when not, 503 on retry/clear without a store, READ delegation, retry outcome recording). `goodsIssueService.test.js` boots `cds.test` so its live queue tests have a bound store. New `test/integration/wm/goodsIssueQueue.test.js` exercises the real OData endpoints: summary, queueing via `postGoodsIssue`, `$filter`/`$count` on the entity set, rejection of direct POSTs to the entity set, retry/clear actions, and the fail-closed 501 response without a store.
  - **Validation**:
    - `npx cds compile srv --to json`: 0 errors; `GoodsIssueQueue` projection carries `@readonly`; `QueueSummary` has `QueuedCount, TotalCount, StoreAvailable, Items`.
    - `npm test` (unit + integration + e2e): **60 suites, 746 tests, all passed** (before the new integration file was added).
    - `npx jest test/integration/wm/goodsIssueQueue.test.js`: **5 passed** (one assertion corrected: OData V4 serialises `Edm.Decimal` as a string).
    - Production-profile startup probe (`NODE_ENV=production`, placeholder XSUAA binding, no database): `cds-serve` starts and `GET /health` returns 200, so the read-only projection on a database entity does not break boot without a database.
    - `git diff --check`: clean.
  - **Not validated**: no Cloud Foundry deployment with a real database service; local development uses the in-memory SQLite database of the development profile, so the queue is empty after every `cds watch` restart until a file-based SQLite (`cds deploy --to sqlite`) or a bound database is configured.
  - **Blocker / decision required**: the production store must be provisioned before the queue works on Cloud Foundry. Options: (a) SAP HANA Cloud HDI container (`cds add hana`, `@cap-js/hana`, `hdi-container` resource plus `db-deployer` module in `mta.yaml`; anticipated by ADR-0001, needs a HANA Cloud instance mapped to the space), (b) PostgreSQL on SAP BTP (`cds add postgres`, `@cap-js/postgres`, `postgresql-db` service; cheaper, needs an ADR addendum), or (c) remove the queue feature entirely (entity, actions, tray UI, tests). Until decided, deployed instances return the SAP error transparently and record nothing.
  - **Next recommended action**: choose the store; then `cds add hana` or `cds add postgres`, add the MTA resource/deployer, add `[production]` `requires.db`, deploy, and re-run the integration tests against the bound database.

## 2026-09-14 15:58 IST
- **Agent**: Claude Code (Fable 5.1)
- **Change**: Routed all warehouse (WM) and EWM traffic to S/4HANA through one shared SAP Cloud SDK based client and removed the raw `fetch` transport, per-adapter `.env` loaders and adapter-level SAP session state (`srv/integration/s4hana/S4HttpClient.js` new, `srv/integration/s4hana/localEnv.js` new, `srv/integration/s4hana/wm/GoodsIssueAdapter.js`, `srv/integration/s4hana/wm/GoodsReceiptAdapter.js`, `srv/integration/s4hana/ewm/EwmAdapter.js`, `server.js`, `test/unit/s4HttpClient.test.js` new, `test/unit/wm/goodsReceiptService.test.js`, `WORKSTATUS.md`).
  - **Root cause**: `GoodsIssueAdapter`, `GoodsReceiptAdapter` and `EwmAdapter` called S/4HANA with Node's global `fetch`, building Basic-auth headers by hand. The BTP destination template (`config/destinations/destination-service.json`) declares `ProxyType: OnPremise`, which requires the Connectivity proxy and proxy-authorization headers that only the Cloud SDK `executeHttpRequest` injects, so those modules could never reach an on-premise S/4HANA from Cloud Foundry. The same three adapters also kept `this.cookie` / `this.csrfToken` on their singleton instance (SAP sessions shared between concurrent users), each carried its own copy of a `.env` parser, and `GoodsReceiptAdapter` had a hard-coded fallback host (`http://172.27.100.32:8000`).
  - **Resolution**:
    1. `S4HttpClient` (new): the single client for direct S/4HANA calls. `resolveDestination()` resolves in order SDK `getDestination` (BTP Destination service or the destination registered in `server.js`), then `S4_DESTINATION_URL`/`S4_USERNAME`/`S4_PASSWORD`/`S4_CLIENT`, then the origin of configured remote-service credentials in cds.env. `get()`, `getText()` (for `$metadata`) and `post()` all run through `executeHttpRequest` with the SDK's own CSRF handling disabled; `post()` fetches the CSRF token and SAP session cookies with one GET immediately before each request (default: the target service root; adapters may pass an explicit `csrfPath`) and sends them with that request only, using the existing `SessionContext`. Failures are normalised to `S4HttpError` with `status`, `statusCode`, SAP OData `code`, message `S/4HANA <METHOD> <path> failed: HTTP <status> - <SAP message>` and the response body, so `S4ErrorMapper` keeps working. No hard-coded SAP client: `sap-client` comes from the destination (`sapClient` / destination headers) or `S4_CLIENT`.
    2. `localEnv.js` (new): one idempotent, non-production loader for `.env.local` then `.env` (existing variables are never overridden). Used by `server.js` (replacing its inline parser) and by the client when running outside the CAP server (unit tests).
    3. The three adapters keep their public and test-facing methods (`_get`, `_post`, `_getMetadataXml`, `_getDestination`) as thin wrappers over `this.client`; result unwrapping and error mapping are unchanged per adapter (Goods Issue and EWM map through `S4ErrorMapper.mapS4Error`, Goods Receipt exposes the SAP message with `statusCode`). Removed from them: `fetch`, `_fetchCsrfToken`, `_ensureEnvLoaded`, `_getCredentials`, `this.cookie`, `this.csrfToken`, the hard-coded host, and every `'220'` default. `GoodsIssueAdapter` and `GoodsReceiptAdapter` keep their proven CSRF probe (`MMIM_GR4PO_DL_SRV/HMmimGr4inbdelSet?$top=1`) as `static CSRF_FETCH_PATH`; EWM probes the target service root as before. Adapters accept an injected `client` for tests.
    4. `test/unit/s4HttpClient.test.js` (new, 17 tests): service-root derivation for V2 sap/scwm and V4 paths, query joining, dotenv parsing and precedence, destination fallback order, GET wiring (Accept, sap-client, `fetchCsrfToken:false`), `$metadata` as text, error normalisation for wrapped SDK errors and network failures, unresolved-destination handling, POST CSRF probe + token/cookie propagation, explicit `csrfPath`, continuing without a token when the probe fails, and no token/cookie retained between calls.
    5. `test/unit/wm/goodsReceiptService.test.js`: the CSRF test now calls `GoodsReceiptAdapter.client.fetchCsrfSession(...)` and asserts the adapter itself holds no token or cookie.
  - **Validation**:
    - Transport probe against a local HTTP server through `executeHttpRequest` (scratch script): raw OData `$filter` strings with spaces and quotes are percent-encoded by the SDK exactly as `fetch` did; `x-csrf-token` and `set-cookie` arrays arrive intact; `$metadata` stays a string with `responseType: 'text'`; network failures carry `code: ECONNREFUSED`.
    - `npm test` (unit + integration + e2e, `--runInBand`): **60 suites, 744 tests, all passed**. This includes the pre-existing live suites in `test/unit/wm/goodsReceiptService.test.js` and the "Verified Real SAP S/4HANA Integration Tests (Client 220)" block of `goodsIssueService.test.js`, which now reach DS4 client 220 through the Cloud SDK path (reads via `MMIM_GR4PO_DL_SRV`, `LO_BM_BATCH_SRV`, `UI_RESERVATION_ITM_MNG_V2`, `MMIM_MATERIAL_DATA_SRV`; CSRF token + `sap-client=220` session cookie obtained via `fetchCsrfSession`).
    - `npx cds compile srv --to json`: 0 errors. `git diff --check`: clean. Repo sweep: no `fetch(` call and no adapter-level `cookie`/`csrfToken` state left under `srv/integration` (only `SessionContext` fields).
  - **Not validated**: no deployment to Cloud Foundry with a real on-premise destination and Cloud Connector was performed, so the Connectivity-proxy path itself was not observed live; only the SDK call path and the local Basic-auth destination were exercised.
  - **Findings recorded for follow-up**:
    - `test/unit/wm/goodsReceiptService.test.js` and one block of `goodsIssueService.test.js` are live S/4 tests that depend on `.env.local` credentials and network access; they will fail offline or in CI and belong under `test/integration` behind an explicit opt-in.
    - `PurchaseOrderAdapter`, `SalesInquiryAdapter` and `AuthAdapter` already use the Cloud SDK but still carry their own destination resolution, an `.env` loader (PO) and `'220'` defaults; folding them onto `S4HttpClient.resolveDestination()` is the next consolidation step.
  - **Next recommended action**: item 5 of the 2026-09-14 review (file-based goods-issue queue on ephemeral Cloud Foundry disk with two instances, no database in production) or item 6 (EWM handler fabricating warehouse tasks on SAP rejection).
